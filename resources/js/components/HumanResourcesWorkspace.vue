...
<template>
    ...
    <div v-if="credentials" class="modal-backdrop"><section class="modal credential-modal" role="alertdialog" aria-modal="true"><header class="modal__header"><div><p class="eyebrow">Cuenta creada</p><h2>Guarde estas credenciales ahora</h2></div></header><div class="credential-content"><p>La contrasena temporal se muestra una unica vez. La persona debe cambiarla en su primer acceso.</p><div><span>Correo SIGAL</span><strong>{{ credentials.email }}</strong></div><div><span>Contrasena temporal</span><strong class="credential-password">{{ credentials.temporary_password }}</strong></div></div><footer class="modal__actions"><button class="button button--primary" type="button" @click="credentials = null">Ya las guarde</button></footer></section></div>
